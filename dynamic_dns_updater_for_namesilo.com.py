...
# record_fqdn = record + '.' + domain if record else domain
record_ttl = '&rrttl=3600'


def update_records():
    print('Record name: ' + record_host + '\nRecord\'s IP: ' + record_ip)
    if callers_ip != record_ip:
        print('Update required.')
        record_id_param = '&rrid=' + record_id
        record_ip_param = '&rrvalue=' + callers_ip
        update_record = ''.join(
            [namesilo_api, 'dnsUpdateRecord', version_param, resp_type_param, api_key_param, domain_param, record_host,
